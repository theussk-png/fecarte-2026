...
dadoOkBtn.addEventListener('click', () => {
    ...
    if (typeof FX !== 'undefined') {
        FX.go({
            btn: null,
            option: {},
            srcScene: story[playerState.currentScene],
            destScene: story[next],
            sameLoc: LOCATIONS[playerState.currentScene] === LOCATIONS[next],
            swap: () => showScene(next)
        });
    } else {
        showScene(next);
...
function applyMoodAndEvent(sceneKey, scene) {
    ...
    // Estado da moldura vermelha (normal / tensão / perigo / sobrenatural)
    if (typeof FX !== 'undefined') {
        FX.frameClasses(scene).forEach(c => gameContainer.classList.add(c));
    }
    if (scene.mood) {
...
function showScene(sceneKey) {
    ...
    // Limpa as opções antigas (elas ficam visíveis durante a transição)
    ...
    if (typeof FX !== 'undefined') FX.enterScene(sceneKey, scene);
}
...
function renderOptions(options) {
    ...
    options.forEach(option => {
        ...
        // O texto vai num <span> para poder "afundar" separado do botão
        const label = document.createElement('span');
        label.className = 'btn-label';
        label.textContent = option.text;
        btn.appendChild(label);
        btn.onclick = () => handleOption(option, btn);
        optionsContainer.appendChild(btn);
    });
}

function handleOption(option, btn) {
    if (isTyping) {
        skipTyping();
        return;
    }
    ...
    // (os botões agora ficam na tela durante a animação de escolha, mas
    // a trava acima + a classe "locked" do FX impedem novos cliques)
    ...
    if (option.roll) {
        ...
        const showDado = () => {
            // Remove os botões para que um segundo clique não tenha em que clicar
            optionsContainer.innerHTML = '';
            ...
            if (typeof FX !== 'undefined') FX.dieResult(success);
            // isProcessingOption continua travado até o clique em OK (ver listener do dado-ok-btn)
        };

        if (typeof FX !== 'undefined') FX.rollChoice(btn, option).then(showDado);
        else showDado();
    } else if (option.next) {
        // isProcessingOption fica travado até showScene() rodar (fim da transição)
        const dest = story[option.next];
        if (typeof FX !== 'undefined') {
            FX.go({
                btn,
                option,
                srcScene: story[playerState.currentScene],
                destScene: dest,
                sameLoc: LOCATIONS[playerState.currentScene] === LOCATIONS[option.next],
                swap: () => showScene(option.next)
            });
        } else {
